...
class ProgressTracker:
    ...
    def _prune_locked(self) -> None:
        ...
        while len(self._finished_order) > self._MAX_FINISHED:
            ...
            if old_id in self._runs and self._runs[old_id]["status"] in (
                "completed",
                "failed",
            ):
...
class SimulationService:
    ...
    def __init__(self, db_path: str | None = None) -> None:
        ...
        logger.info(
            "SimulationService initialized with Bangalore Digital Twin topology"
        )
    ...
    def _create_run(
        ...
        # Scenario Library (Phase 10E-2): world-state modifiers applied to the
        # fresh engine/schedule before any execution. Deterministic per seed.
        apply_scenario(scenario, engine, schedule, hospitals)
    ...
    def _build_result_data(
        ...
        """Assemble the persisted run payload with Phase 10E-2 evidence."""
        result: dict[str, Any] = {
            "run_id": run_id,
            "strategy": strategy.name,
            "parameters": params,
            "scenario": {
                "key": params.get("scenario", DEFAULT_SCENARIO),
                "name": scenario_display(params.get("scenario")),
            },
            "metrics": metrics,
        ...
        if hasattr(strategy, "get_mode_stats"):
            result["adaptive_mode_stats"] = strategy.get_mode_stats()
        return result
    ...
    def _run_background(
        ...
        try:
            ...
            self._tracker.set_status(
                run_id, "failed", error="Simulation execution failed"
            )
            self._store.save_run(
                run_id,
                {"run_id": run_id, "strategy": strategy.name},
    ...
    def _monitor_run(
        self, run_id: str, engine: CitySimulationEngine, duration_seconds: float
    ) -> None:
